...
int main() {
    ...
    UnionFind u(n * 2);
    for (int i = 0; i < n; ++i) {
        u.unite(rope_num(i,'R'),rope_num(i,'B'));
    }
    int ans_loop = 0;
    int ans_non_loop = n;
    for (int i = 0; i < m; ++i) {
        ...
        a--;
        c--;
        if(u.same(rope_num(a,b),rope_num(c,d))){
            ans_loop ++;
            ans_non_loop --;
        }else{
            u.unite(rope_num(a,b),rope_num(c,d));
            ans_non_loop --;
        }
    }
    cout<<ans_loop<<" "<<ans_non_loop<<endl;
